...
export const searchSongs = query(z.object({
  term: z.string()
}), async (params) => {
  const token = createDeveloperToken()

  const searchReq = await fetch(`https://api.music.apple.com/v1/catalog/US/search?term=${params.term}&types=songs&limit=25`, {
    headers: {
